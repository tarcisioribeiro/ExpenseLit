...
from utils.date_utils import format_date_for_display, format_currency_br
...
class PDFGenerator:
    ...
    def generate_expense_receipt(
        ...
        expense_info = [
            ...
            ["Valor:", format_currency_br(float(expense_data.get('value', 0)))],
            ...
            ["Status:", "✅ Paga" if expense_data.get('payed', False) else "⏳ Pendente"]
        ]
        ...
        footer_text = f"""
        <para align="center">
        Este comprovante foi gerado automaticamente pelo sistema ExpenseLit<br/>
    ...
    def generate_revenue_receipt(
        ...
        revenue_info = [
            ...
            ["Valor:", format_currency_br(float(revenue_data.get('value', 0)))],
            ...
            ["Status:", "✅ Recebida" if revenue_data.get('received', False) else "⏳ Pendente"]
        ]
    ...
    def generate_loan_contract(
        ...
        loan_info = [
            ...
            ["Valor Total:", format_currency_br(float(loan_data.get('value', 0)))],
            ["Valor Pago:", format_currency_br(float(loan_data.get('payed_value', 0)))],
            ["Saldo Devedor:", format_currency_br(
                float(loan_data.get('value', 0)) - float(loan_data.get('payed_value', 0))
            )],
            ["Data do Empréstimo:", format_date_for_display(loan_data.get('date', ''))],
            ["Horário:", loan_data.get('horary', 'N/A')],
            ["Categoria:", db_categories.EXPENSE_CATEGORIES.get(
                loan_data.get('category', 'others'), 
                loan_data.get('category', 'N/A')
            )],
            ["Status:", "✅ Quitado" if loan_data.get('payed', False) else "⏳ Em Aberto"]
        ]
        ...
        realizado em <b>{format_date_for_display(loan_data.get('date', ''))}</b>.<br/><br/>
        
        <b>2.</b> O devedor compromete-se a quitar o valor emprestado conforme acordado entre as partes.<br/><br/>
        
        <b>3.</b> Este contrato é meramente informativo e foi gerado automaticamente pelo sistema ExpenseLit.<br/><br/>
        
        <b>4.</b> Para questões legais, consulte um advogado especializado em contratos.<br/>
        </para>
        """
    ...
    def generate_transfer_receipt(
        ...
        # Cabeçalho
        title = Paragraph("COMPROVANTE DE TRANSFERÊNCIA", self.styles['CustomTitle'])
        story.append(title)
        story.append(Spacer(1, 20))
        ...
        transfer_info = [
            ...
            ["Valor:", format_currency_br(float(transfer_data.get('value', 0)))],
    ...
    def generate_credit_card_receipt(
        ...
        # Cabeçalho
        title = Paragraph("COMPROVANTE DE DESPESA - CARTÃO DE CRÉDITO", self.styles['CustomTitle'])
        story.append(title)
        story.append(Spacer(1, 20))
        ...
        if expense_data.get('merchant'):
            expense_info.append(["Estabelecimento:", expense_data.get('merchant')])
        
        if expense_data.get('location'):
            expense_info.append(["Local:", expense_data.get('location')])
        
        if expense_data.get('transaction_id'):
            expense_info.append(["ID Transação:", expense_data.get('transaction_id')])
...
try:
    ...
    pdf_generator = None
    logger.warning("PDFGenerator não pôde ser inicializado. ReportLab não está disponível.")
...
months = QueryExecutor().complex_consult_query(months_query)
months = QueryExecutor().treat_numerous_simple_result(months, to_remove_list)

account_models = QueryExecutor().complex_consult_query(account_models_query)
account_models = QueryExecutor().treat_numerous_simple_result(
    account_models, to_remove_list)

years = QueryExecutor().complex_consult_query(years_query)

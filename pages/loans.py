...
class LoansPage(BasePage):
    ...
    def __init__(self):
        ...
        self.required_permissions = ['loans.view_loan']
    ...
    def _render_loans_list(self) -> None:
        ...
        try:
            ...
            paid_loans = sum(
                    float(loan.get('payed_value', 0)) for loan in loans)
        ...
        with st.container():
            ...
            with col4:
                with st.popover("⚙️ Ações"):
                    ...
                    toggle_text = (
                        "⏳ Marcar Pendente" if loan.get('payed', False)
                                                    else "✅ Marcar Quitado")
                    if st.button(
                        toggle_text,
                        ...
                        self._toggle_loan_payment(
                        loan_id, not loan.get('payed', False))
        ...
        with st.form(f"payment_loan_form_{loan_id}"):
            ...
            with col_submit:
                if st.form_submit_button(
                    "💾 Registrar Pagamento", type="primary"):
        ...
        with st.form(f"edit_loan_form_{loan_id}"):
            ...
            with col2:
                ...
                current_date = datetime.strptime(
                    loan.get('date', ''), '%Y-%m-%d').date() if loan.get('date') else datetime.now().date()
                current_time = datetime.strptime(
                    loan.get('horary', '00:00:00'), '%H:%M:%S').time() if loan.get('horary') else datetime.now().time()
                ...
                try:
                    ...
                    if accounts:
                        account_options = [(
                            acc['id'], db_categories.INSTITUTIONS.get(acc['name'], acc['name'])) for acc in accounts if acc.get('is_active', True)]
                ...
                try:
                    ...
                    if members:
                        member_options = [(
                        member['id'], member['name']) for member in members if member.get('active', True)]
    ...
    def _render_give_loan_form(self) -> None:
        """Renderiza formulário para realizar empréstimo (
        emprestar dinheiro)."""
    ...
    def _render_receive_loan_form(self) -> None:
        ...
        st.info(
            "🏦 Você está tomando dinheiro emprestado. O dinheiro entrará na sua conta.")
        ...
        Any]
            ...
            if st.form_submit_button("💾 Criar Empréstimo", type="primary"):
                ...
                if not loan_data.get('confirm_data'):
                    errors.append(
                    "Confirme que os dados estão corretos antes de prosseguir")
                ...
                if not loan_data.get(
                    'creditor_id') and not loan_data.get('benefited_id'):
    ...
    def _calculate_account_balance(self, account_id: int) -> float:
        ...
        try:
            ...
            account_expenses = [e for e in expenses if e.get(
            'account') == account_id]
            ...
            outgoing_transfers = [t for t in transfers if t.get(
            'origin_account') == account_id and t.get('transfered', False)]
            balance -= sum(float(t.get('value', 0)) for t in outgoing_transfers)

            # Transferências onde a conta é destino (soma)
            incoming_transfers = [t for t in transfers if t.get(
            'destiny_account') == account_id and t.get('transfered', False)]
            balance += sum(float(t.get('value', 0)) for t in incoming_transfers)

            # Empréstimos dados (subtrai da conta) - apenas os efetivados
            loans = api_client.get("loans/")
            given_loans = [l for l in loans if l.get(
            'account') == account_id and l.get('loan_type') == 'given']
            balance -= sum(float(l.get('value', 0)) for l in given_loans)

            # Empréstimos recebidos (soma na conta) - apenas os efetivados
            received_loans = [l for l in loans if l.get(
            'account') == account_id and l.get('loan_type') == 'received']
    ...
    def _toggle_loan_payment(self, loan_id: int, is_paid: bool) -> None:
        ...
        try:
            with st.spinner(
                f"{'Marcando como quitado' if is_paid else 'Marcando como pendente'}..."):
    ...
    def _delete_loan(self, loan_id: int, description: str) -> None:
        ...
        if not st.session_state.get(confirm_key, False):
            ...
            st.warning(
            f"⚠️ **Tem certeza que deseja excluir o empréstimo '{description}'?**")
            ...
            with col1:
                if st.button(
                    ...
                    try:
                        ...
                        st.success(
                            f"✅ Empréstimo '{description}' excluído com sucesso!")
    ...
    def _generate_loan_pdf(self, loan: Dict[str, Any]) -> None:
        ...
        if pdf_generator is None:
            st.error(
                "❌ Gerador de PDF não disponível. Instale o ReportLab: pip install reportlab")
            return
        ...
        try:
            with st.spinner("📄 Gerando contrato..."):
                ...
                try:
                    members = api_client.get("members/")
                    creditor_data = next(
                    (m for m in members if m['id'] == loan.get('creditor')), None)
                    benefited_data = next(
                    (m for m in members if m['id'] == loan.get('benefited')), None)
                except ApiClientError:
                        pass

                    # Gerar PDF
                pdf_buffer = pdf_generator.generate_loan_contract(
                    loan,
                    creditor_data,
                    benefited_data
                )
            ...
            logger.error(f"Erro ao gerar contrato de empréstimo: {e}")

...
class NewLoan:
    ...
    def take_new_loan(self):
        ...
        if len(user_current_accounts) == 0:
            ...
            creditors_quantity_query = '''SELECT COUNT(id_credor) FROM credores WHERE credores.documento <> {} OR credores.nome <> '{}';'''.format(
            ...
            creditors_quantity = int(QueryExecutor().treat_simple_result(
                creditors_quantity, to_remove_list))
            ...
            if creditors_quantity == 0:
                ...
                with col1:
                    ...
                    with st.expander(label="Dados", expanded=True):
                        ...
                        final_total_account_revenue_query = total_account_revenue_query.format(
                            account, logged_user, logged_user_password)
                        final_total_account_expense_query = total_account_expense_query.format(
                            account, logged_user, logged_user_password)
                        ...
                        with col2:
                            ...
                            with st.expander(label="Informações", expanded=True):
                                str_selected_account_revenues = (
                                    QueryExecutor().simple_consult_query(final_total_account_revenue_query))
                                ...
                                str_selected_account_expenses = (
                                    QueryExecutor().simple_consult_query(final_total_account_expense_query))
    ...
    def make_new_loan(self):
        ...
        if len(user_current_accounts) == 0:
            ...
            benefited_quantity_query = '''SELECT COUNT(id_beneficiado) FROM beneficiados WHERE beneficiados.documento <> {} OR beneficiados.nome <> '{}';'''.format(
                user_document, user_name)
            ...
            benefited_quantity = int(QueryExecutor().treat_simple_result(
                benefited_quantity, to_remove_list))

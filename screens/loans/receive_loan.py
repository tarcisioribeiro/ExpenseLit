        ...
        if len(not_received_loans[0]) >= 1:
            ...
            with col4:
                ...
                loan_data_df = pd.DataFrame(
                    {
                        ...
                        "Valor a Receber": remaining_value,
                        "Data": date,
                        "Categoria": category,
                        "Conta": account,
                        "Devedor": benefited
                ...
                with st.expander(label="Valores", expanded=True):
                    ...
                    usr_accnts = QueryExecutor().treat_numerous_simple_result(
                        usr_accnts,
                        TO_REMOVE_LIST
                    )
                    ...
                    INNER JOIN
                    contas AS c ON
                        c.proprietario_conta = e.credor
                        AND c.documento_proprietario_conta = e.documento_credor
                    INNER JOIN
                        usuarios ON usuarios.nome = e.credor
                        AND usuarios.documento = e.documento_credor
                    WHERE
                        e.pago = 'N'
                        AND e.descricao = '{}';
                    '''.format(debt)
                    ...
                    INNER JOIN
                        contas AS c
                        ON c.proprietario_conta = e.credor
                        AND c.documento_proprietario_conta = e.documento_credor
                    INNER JOIN
                        usuarios ON usuarios.nome = e.credor
                        AND usuarios.documento = e.documento_credor

...
class Creditors:
    ...
    """
    def is_creditor_new_values_valid(self, creditor_id: int, creditor_new_name: str, creditor_new_document: str, creditor_new_phone: str):
        """
        Realiza a verificação dos novos dados passados do credor. Caso sejam válidos e não utilizados, é retornada a resposta positiva, caso contrário, é retornada uma resposta negativa.
        ...
        if is_new_document_valid == "1":
            st.error(body="O documento {} já está em uso.".format(creditor_new_document))
        if is_new_phone_valid == "1":
            st.error(body="O telefone {} já está em uso.".format(creditor_new_phone))
    ...
    def update_creditor(self):
        ...
        if creditors_quantity == 0:
            ...
            with col4:
                selected_creditor = st.selectbox(label="Beneficiado", options=creditors)
            ...
            with col1:
                ...
                with st.expander(label="Dados atuais de {}".format(selected_creditor), expanded=True):
                    st.write("**Nome**")
            ...
            if confirm_selection:
                ...
                with col2:
                    ...
                    with st.expander(label="Novos dados de {}".format(selected_creditor), expanded=True):
                        new_name = st.text_input(label="**Novo nome**", max_chars=100, help="Novo nome do credor. Preferível que seja o nome completo.", placeholder=creditors_complete_data[1])
                        new_document = st.text_input(label="**Novo documento**", help="Novo CPF/CNPJ do credor, sem barras, pontos e hifens.", placeholder=creditors_complete_data[2])
                        new_phone = st.text_input(label="**Novo telefone**", max_chars=11, help="Novo telefone do credor, apenas números.", placeholder=creditors_complete_data[3])
                    ...
                    if confirm_changes_button:
                        ...
                        with col3:
                            ...
                            with st.expander(label="Validação", expanded=True):
                                is_data_passed_valid = self.is_creditor_new_values_valid(int(creditors_complete_data[0]), new_name, new_document, new_phone)
                            ...
                            if is_data_passed_valid == True:
                                ...
                                if len(loans_ids) >= 1:
                                    ...
                                    ids = tuple(ids)
                                    new_creditor_loans_data_query = """UPDATE emprestimos SET credor = '{}', documento_credor = {} WHERE credor = '{}' AND documento_credor = {} AND id_emprestimo IN{};""".format(
                                        new_name, new_document, creditors_complete_data[1], creditors_complete_data[2], ids)
                                    QueryExecutor().update_table_unique_register(new_creditor_loans_data_query, "Registros atualizados com sucesso!", "Erro ao atualizar registros:")

                                new_creditor_data_query = """UPDATE credores SET nome = '{}', documento = {}, telefone = '{}' WHERE id_credor = {};""".format(
                                    new_name, new_document, new_phone, int(creditors_complete_data[0]))
                                QueryExecutor().update_table_unique_register(new_creditor_data_query, "Credor atualizado com sucesso!", "Erro ao atualizar credor:")
                                ...
                                log_values = (today, actual_time, logged_user, "Atualização", "O usuário atualizou o credor {}.".format(creditors_complete_data[1]))
    ...
    def new_creditor(self):
        ...
        with col1:
            ...
            with st.expander(label="Dados do credor", expanded=True):
                creditor_name = st.text_input(label=":lower_left_ballpoint_pen: Nome", max_chars=100, help="Nome do credor. Recomendável que seja o nome completo.")
                creditor_document = st.text_input(label=":lower_left_ballpoint_pen: Documento", help="CPF/CNPJ do credor, sem pontos e barras.")
                creditor_phone = st.text_input(label=":telephone_receiver: Telefone/Celular", max_chars=11, help="Número de telefone do credor. Informe apenas números.")
            ...
            if confirm_creditor_data and register_new_creditor:
                with col2:
                    ...
                    with st.expander(label="Aviso", expanded=True):
                        ...
                        is_document_valid = Documents().validate_owner_document(creditor_document)
                        ...
                        if is_document_valid == True and creditor_name != '' and creditor_phone != '':
                            ...
                            if (creditor_name != user_name) and (creditor_document != int(user_document)):
                                ...
                                QueryExecutor().insert_query(insert_creditor_query, query_values, "Credor cadastrado com sucesso!", "Erro ao cadastrar credor:")
    ...
    def main_menu(self, menu_position):
        ...
        with menu_position:
            selected_option = st.selectbox(label="Menu de Credores", options=menu_options.keys())

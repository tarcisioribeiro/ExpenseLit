...
class UpdateCreditCards:
        ...
        if len(user_current_accounts) == 0:
            ...
            with col1:
                ...
                with st.expander(label="Dados cadastrais", expanded=True):
                    card_name = st.text_input(label=":lower_left_ballpoint_pen: Nome do cartão", help="Nome descritivo do cartão.")
                    ...
                    owner_name = st.text_input(label=":lower_left_ballpoint_pen: Nome do Titular", help="Nome do titular no cartão.")
            ...
            with col2:
                ...
                with st.expander(label="Dados confidenciais", expanded=True):
                    security_code = st.text_input(label=":lock: Código de segurança", max_chars=3, type="password", help="Código CVV do cartão.")
                        ...
                        with cm_cl2:
                            ...
                            if card_name != "" and card_number != "" and owner_name != "" and security_code != "" and confirm_security_code != "" and (security_code == confirm_security_code) and (today_datetime < expire_date):
                                if is_document_valid == True and is_card_valid == True:
                                    ...
                                    QueryExecutor().insert_query(new_credit_card_query, new_credit_card_values, "Cartão cadastrado com sucesso!", "Erro ao cadastrar cartão")
        ...
        if confirm_values and send_data_button:
            ...
            with col2:
                with st.spinner(text='Aguarde...'):
                    ...
                    QueryExecutor().update_table_unique_register(new_limit_query, "Limite atualizado com sucesso!", "Erro ao atualizar limite:")
                    ...
                    with col2:
                        ...
                        with st.expander(label="Aviso", expanded=True):
                            ...
                            if (beggining_invoice_date < ending_invoice_date) and beggining_invoice_date <= credit_card_expire_date_formatted and ending_invoice_date <= credit_card_expire_date_formatted:
                                ...
                                QueryExecutor().insert_query(new_credit_card_invoice_query, new_credit_card_invoice_values, "Fechamento cadastrado com sucesso!", "Erro ao cadastrar fechamento:")
    ...
    def main_menu(self, menu_position):
        ...
        with menu_position:
            cc_selected_option = st.selectbox(label="Menu de Cartões de Crédito", options=cc_menu_options)

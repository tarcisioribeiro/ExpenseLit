...
class UpdateCreditCards:
                ...
                if send_form_button:
                    with col3:
                        with st.expander(label="Validação dos dados", expanded=True):
                            is_card_valid = self.call_document.validate_credit_card(
                                card_number=card_number
                            )
                            is_document_valid = (
                                self.call_document.validate_owner_document(
                            ...
                            if (
                                ...
                                if is_document_valid == True and is_card_valid == True:
                                    ...
                                    log_values = (
                                        logged_user,
                                        "Cadastro",
                                        "Cadastrou o cartão {} associado a conta {}.".format(
                                    ...
                                    is_document_valid == False and is_card_valid == True
                                ):
                                    st.success(body="Número de cartão válido.")
                                ...
                                st.error(
                                    body="Algum dado não foi informado. Revise-os."
                                )
    ...
    def update_credit_card(self):
        ...
        credit_cards = query_executor.treat_numerous_simple_result(
        ...
        with col1:
            ...
            cc_max_limit = query_executor.treat_simple_result(
            ...
            with st.expander(label="Dados", expanded=True):
                ...
                new_limit = st.number_input(
                    label="Limite", min_value=0, max_value=cc_max_limit, step=1
                )
            ...
            with col2:
                with st.spinner(text="Aguarde..."):
                    ...
                    log_values = (
                        logged_user,
                        "Registro",
                        "Atualizou o limite do cartão {}.".format(card),
                    )
                    query_executor.insert_query(
        ...
        if len(credit_cards) == 0:
            ...
            with col1:
                with st.expander(label="Dados da fatura", expanded=True):
                    ...
                    card_number, owner_name, owner_document, card_code = (
                        call_credit_card.credit_card_key(card=card_name)
                    )
                    ...
                    with col2:
                        with st.expander(label="Validação dos dados", expanded=True):
                            ...
                            if (
                                card_name != ""
                                and month != ""
                                and beggining_invoice_date < ending_invoice_date
                            ):
                                ...
                                log_values = (
                                    logged_user,
                                    "Cadastro",
                                    "Cadastrou um fechamento do cartão {}.".format(
                                        card_name
                                    ),
                                )
                                query_executor.insert_query(
                                ...
                                if beggining_invoice_date >= ending_invoice_date:
                                    ...
                                    st.error(
                                        body="A data de ínicio da fatura não pode ser superior a data do fim da fatura."
                                    )
    ...
    def credit_cards_interface(self):

...
class ConfirmRevenue:
    """
    Representa a tela que realiza a confirmação do recebimento de receitas.

    Attributes
    ----------
    get_not_received_revenue_id(description, value, date, time, category, account)
        Realiza a consulta e retorno do ID da receita que corresponde aos dados passados.
    update_not_received_revenues(id)
        Atualiza a receita não recebida, confirmando o seu recebimento.
    confirm_revenue()
        Apresenta a tela de confirmação do recebimento das receitas, as atualizando assim que o usuário a selecione e confirme como recebidas.
    """
    ...
    def update_not_received_revenues(self, id: int):
        """
        Atualiza as receitas não recebidas, as confirmando como recebidas.

        Parameters
        ----------
        id: int
            O id da despesa a ser atualizada.
        """
            ...
            with col4:
                ...
                with st.expander(label="Dados", expanded=True):
                    ...
                    for i in range(0, len(description)):
                        ...
                        unformatted_data = datetime.strptime(
                            unformatted_data, "%Y-%m-%d"
                        )
                ...
                if confirm_selection and update_button:
                    ...
                    with col6:
                        ...
                        self.update_not_received_revenues(id=final_id)
                        ...
                        log_values = (
                            logged_user,
                            "Registro",
                            "Registrou uma receita no valor de R$ {} associada a conta {}.".format(
                                value, account
                            ),
                        )

    ...
    def __init__(self):
        ...
        def mount_card_graph(selected_card: str):
            ...
            month_expenses = credit_card.month_expenses(
                selected_card, int_actual_year, string_actual_month)
            ...
            st.dataframe(credit_card_data_df, hide_index=True,
        ...
        def mount_balance():
            ...
            final_str_ticket_ammount = variable.treat_complex_string(
                ticket_ammount)
            values_list.append(final_str_ticket_ammount)

            fund_revenue_ammount = query_executor.simple_consult_query(
                fund_revenue_query)
            ...
            fund_expense_ammount = query_executor.simple_consult_query(
                fund_expense_query)
            ...
            final_str_fund_ammount = variable.treat_complex_string(
                fund_ammount)
            values_list.append(final_str_fund_ammount)

            loan_ammount = query_executor.simple_consult_query(
                loan_expense_query)
            loan_ammount = query_executor.treat_simple_result(
                loan_ammount, to_remove_list)
            final_str_loan_ammount = variable.treat_complex_string(
                loan_ammount)
            values_list.append(final_str_loan_ammount)

            debts_ammount = query_executor.simple_consult_query(
                debts_expense_query)
            debts_ammount = query_executor.treat_simple_result(
                debts_ammount, to_remove_list)
            final_str_debts_ammount = variable.treat_complex_string(
                debts_ammount)
        ...
        def show_balance():
            ...
            with col2:
                ...
                with col22:
                    ...
                    if user_sex == "F":
                        st.subheader(body="Bem vind{}, {}!".format(
                            "a", str_user_name[0]))
                    elif user_sex == "M":
                        st.subheader(body="Bem vind{}, {}!".format(
                            "o", str_user_name[0]))
            ...
            with col3:
                ...
                with col32:
                    ...
                    st.info(body=":spiral_calendar_pad: {}".format(
                        app_actual_date.strftime("%d/%m/%Y")))
                with col33:
                    app_actual_horary = datetime.now().strftime("%H:%M:%S")
                    st.info(body=":mantelpiece_clock: {}".format(
                        app_actual_horary))
            ...
            with col4:
                ...
                with st.expander(label=":chart: Valores principais", expanded=True):
                    st.text(body="O seu saldo total é:",
                            help="Valor considerando saldo em contas correntes.")
                    st.info(body=":heavy_dollar_sign: {}".format(
                        balance_values_list[0]))
                    st.text(body="Saldo de Vale Disponível:",
                            help="Valor considerando saldo de vales alimentação e refeição.")
                    st.info(body=":heavy_dollar_sign: {}".format(
                        balance_values_list[1]))
                    st.text(body="Fundo de garantia:",
                            help="Valor considerando saldos de fundo de garantia.")
                    st.info(body=":heavy_dollar_sign: {}".format(
                        balance_values_list[2]))
                    st.text(body="Empréstimos não recebidos:",
                            help="Valores de empréstimos realizados que ainda não recebidos.")
                    st.info(body=":heavy_dollar_sign: {}".format(
                        balance_values_list[3]))
                    st.text(body="Valores a pagar:",
                            help="Dívidas e empréstimos contraídos que ainda não foram pagos.")
                    st.info(body=":heavy_dollar_sign: {}".format(
                        balance_values_list[4]))
                ...
                with st.expander(label=":bank: Contas", expanded=True):
                    if balance > 0 and len(accounts_balance) > 0:
                        ...
                        balance_data_df["Valor"] = balance_data_df["Valor"].apply(
                            lambda x: f"R$ {x:.2f}".replace(".", ","))
                        balance_data_df["Valor Futuro"] = balance_data_df["Valor Futuro"].apply(
                            lambda x: f"R$ {x:.2f}".replace(".", ","))
                        st.data_editor(
            ...
            with col5:
                ...
                with st.expander(label=":scroll: Últimos registros", expanded=True):
                    ...
                    if len(last_revenues) > 0:
                        revenue_df = pd.DataFrame(last_revenues, columns=[
                                                  "Descrição", "Valor", "Data", "Categoria", "Conta"])
                        revenue_df["Valor"] = revenue_df["Valor"].apply(
                            lambda x: f"R$ {x:.2f}".replace(".", ","))
                        revenue_df["Data"] = pd.to_datetime(
                            revenue_df["Data"]).dt.strftime("%d/%m/%Y")
                        st.dataframe(revenue_df, hide_index=True,
                                     use_container_width=True)
                    else:
                        st.warning(
                            body="Você ainda não possui receitas registradas.")

                    st.info(body=":arrow_down_small: Últimas despesas")

                    if len(last_expenses) > 0:
                        expense_df = pd.DataFrame(last_expenses, columns=[
                                                  "Descrição", "Valor", "Data", "Categoria", "Conta"])
                        expense_df["Valor"] = expense_df["Valor"].apply(
                            lambda x: f"R$ {x:.2f}".replace(".", ","))
                        expense_df["Data"] = pd.to_datetime(
                            expense_df["Data"]).dt.strftime("%d/%m/%Y")
                        st.dataframe(expense_df, hide_index=True,
                                     use_container_width=True)
                    else:
                        st.warning(
                            body="Você ainda não possui despesas registradas.")
                ...
                with st.expander(label=":scroll: Maiores registros", expanded=True):
                    ...
                    if len(max_revenue) > 0:
                        max_revenue_df = pd.DataFrame(
                            max_revenue, columns=["Descrição", "Valor", "Data", "Categoria", "Conta"])
                        max_revenue_df["Valor"] = max_revenue_df["Valor"].apply(
                            lambda x: f"R$ {x:.2f}".replace(".", ","))
                        max_revenue_df["Data"] = pd.to_datetime(
                            max_revenue_df["Data"]).dt.strftime("%d/%m/%Y")
                        st.dataframe(max_revenue_df, hide_index=True,
                                     use_container_width=True)
                    else:
                        st.warning(
                            body="Você ainda não possui receitas registradas.")
                    ...
                    if len(max_expense) > 0:
                        ...
                        max_expense_df["Data"] = pd.to_datetime(
                            max_expense_df["Data"]).dt.strftime("%d/%m/%Y")
                        st.dataframe(max_expense_df, hide_index=True,
                                     use_container_width=True)
                    else:
                        st.warning(
                            body="Você ainda não possui despesas registradas.")
            ...
            with col6:
                ...
                with st.expander(label=":bar_chart: Contas Correntes", expanded=True):
                    ...
                    if len(accounts_expense_graph) > 0:
                        ...
                        legend_labels = [
                            "{} - {:.2f}%".format(
                                category, (value / account_df["Valor"].sum()) * 100).replace(".", ",")
                        ...
                        ax.set_title("Despesas por Categoria - Contas Correntes",
                                     fontsize=12, fontproperties=custom_font)
                ...
                with st.expander(label=":bar_chart: Cartão de Crédito", expanded=True):
                    ...
                    if len(credit_card_expense_graph) > 0:
                        ...
                        legend_labels = [
                            "{} - {:.2f}%".format(
                                category, (value / credit_card_df["Valor"].sum()) * 100).replace(".", ",")

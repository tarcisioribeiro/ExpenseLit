...
class AccountStatement:
    ...
    def mount_statement_query(
        self, statement_type: str, accounts: list, initial_data: str, final_data: str
    ):
        """
    ...
    def consult_statement(self, statement_query_list: list):
        ...
        for i in range(len(statement_query_list)):
            empty_list = query_executor.complex_compund_query(
                statement_query_list[i], 6, "statement_"
            )
        ...
        if confirm_choice and consult_tables:
            ...
            with col5:
                ...
                if (
                    ...
                    and len(selected_accounts) > 0
                    and (initial_data <= final_data)
                ):
                    ...
                    with st.spinner(text="Aguarde..."):
                        sleep(1)
                    value = self.consult_statement(query_list)
                    ...
                    with col6:
                        ...
                        with st.expander(label="Dados", expanded=True):
                            st.info(
                                body="Quantidade de {}: {}.".format(
                                    statement_option, len(value)
                                )
                            )
                            st.info(
                                body="Valor total das {}: R$ {}.".format(
                                    statement_option, total_value
                                )
                            )
                            st.info(
                                body="Valor médio das {}: R$ {}.".format(
                                    statement_option, medium_value
                                )
                            )
                        ...
                        log_values = (
                            logged_user,
                            "Consulta",
                            "Consultou o relatório de {} entre o período de {} a {}.".format(
                                statement_option, initial_data, final_data
                            ),
                        )

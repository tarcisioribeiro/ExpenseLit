...
class AccountStatement:
    """
    Classe responsável pela montagem do extrato bancário das contas (Despesas x Receitas).

    Attributes
    ----------
    mount_statement_query(statement_type, accounts, initial_data, final_data)
        Realiza a montagem da consulta do extrato.
    consult_statement(statement_query_list)
        Realiza a consulta do extrato com base nas consultas.
    main_menu()
        O menu do extrato bancário.
    """
    ...
    def mount_statement_query(self, statement_type: str, accounts: list, initial_data: str, final_data: str):
        ...
        statement_type: str
            O tipo do extrato.
        accounts: list
            As contas que serão consultadas.
        initial_data: str
        ...
        Returns
        -------
        statement_query_list: list
            A lista com as consultas montadas.
        """
        ...
        if confirm_choice and consult_tables:
            ...
            with col5:
                ...
                if statement_option != "Selecione uma opção" and len(selected_accounts) > 0 and (initial_data <= final_data):
                        query_list = self.mount_statement_query(statement_option, selected_accounts, initial_data, final_data)

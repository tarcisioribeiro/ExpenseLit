...
class Receipts:
    ...
    def validate_query(self, table: str, date: str, account: str, value: float):
        ...
        -------
        ids_string : Any
            A string com os id's que correspondem a consulta, caso a mesma seja válida.
        boolean
    ...
    def execute_query(self, table: str, id_list: list):
        ...
        if table == "despesas_cartao_credito":
            values_query = """SELECT descricao, valor, data, horario, categoria, cartao FROM despesas_cartao_credito WHERE id_despesa_cartao IN %s;"""
            values_query = values_query.replace("IN %s", f"IN ({placeholders})")
        elif table == "receitas":
            values_query = """SELECT descricao, valor, data, horario, categoria, conta FROM receitas WHERE id_receita IN %s;"""
            values_query = values_query.replace("IN %s", f"IN ({placeholders})")
        elif table == "despesas":
            values_query = """SELECT descricao, valor, data, horario, categoria, conta FROM despesas WHERE id_despesa IN %s;"""
            values_query = values_query.replace("IN %s", f"IN ({placeholders})")

        consult_values = QueryExecutor().complex_compund_query(values_query, 6, params=(*id_list,))
    ...
    def treat_receipt_values(self, receipt_list: list):
        ...
        if len(receipt_list) >= 5 and len_lists_receipt >= 5:
            ...
            for i in range(0, len(description)):
                aux_description = QueryExecutor().treat_simple_result(description[i], to_remove_list)
            ...
            for i in range(0, len(value)):
                aux_value = QueryExecutor().treat_simple_result(value[i], to_remove_list)
            ...
            for i in range(0, len(date)):
                aux_date = QueryExecutor().treat_simple_result(date[i], to_remove_list)
            ...
            for i in range(0, len(category)):
                aux_category = QueryExecutor().treat_simple_result(category[i], to_remove_list)
            ...
            for i in range(0, len(account)):
                aux_account = QueryExecutor().treat_simple_result(account[i], to_remove_list)
    ...
    def generate_transfer_receipt(self, id: int, description: str, value: float, date: str, category: str, origin_account: str, destiny_account: str):
        ...
        destiny_account_image = QueryExecutor().simple_consult_query(query=account_image_query, params=(destiny_account, user_name, user_document))
        destiny_account_image = QueryExecutor().treat_simple_result(destiny_account_image, to_remove_list)
        ...
        if operational_system == "nt":
            ...
            header_font = ImageFont.truetype("{}{}".format(absolute_app_path, system_font), font_size,)
    ...
    def generate_receipt(self, table: str, id: int, description: str, value: float, date, category: str, account: str):
        ...
        with open(caminho_arquivo, "rb") as file:
            download_button = st.download_button(label=":floppy_disk: Baixar imagem", data=file, file_name="Relatorio_{}_{}.png".format(today, actual_horary),)
                        ...
                        if is_query_valid == True:
                            ...
                            with st.expander(label=":bar_chart: Resultados", expanded=True):
                                ...
                                for i in range (0, len(query_data)):
                                    if i == (len(query_data) - 1):
                                        ids_string += "ID " + str(query_data[i])
                                    else:
                                        ids_string += "ID " + str(query_data[i]) + ", "
                                

                                st.info("Registro(s) encontrado(s): {}.".format(ids_string))
                                ...
                                select_id_register = st.selectbox(label="Selecione o ID do registro desejado", options=ids_list)
                                id_list_index = ids_list.index(select_id_register)
                            ...
                            if confirm_register_selection == True and generate_receipt_button:
                                with col6:
                                    ...
                                    self.generate_receipt(table, select_id_register, description[id_list_index], value[id_list_index], date[id_list_index], category[id_list_index], account[id_list_index])

...
class Receipts:
    ...
    def execute_query(self, table, id):
        """
        ...
        if table == "despesas_cartao_credito":
            values_query = """SELECT id_despesa_cartao, descricao, valor, data, categoria, cartao FROM {} WHERE id_despesa_cartao = {};""".format(
                table, id
            )
        elif table == "receitas":
            values_query = """SELECT id_receita, descricao, valor, data, categoria, conta FROM {} WHERE id_receita = {};""".format(
                table, id
            )
        elif table == "despesas":
            values_query = """SELECT id_despesa, descricao, valor, data, categoria, conta FROM {} WHERE id_despesa = {};""".format(
                table, id
            )

        consult_values = query_executor.complex_compund_query(
            values_query, 6, "query_values"
        )

        return consult_values

    def treat_receipt_values(self, receipt_list):
        """
        ...
        if len(receipt_list) >= 5 and len_lists_receipt >= 5:
            ...
            description = query_executor.treat_simple_result(
                description, to_remove_list
            )
    ...
    def generate_transfer_receipt(
        ...
        caminho_arquivo = (
            "{}/data/receipts/transfers/Comprovante_de_transferencia_{}_{}.png".format(
    ...
    def generate_receipt(self, table, id, description, value, date, category, account):
        """
                ...
                with col5:
                    ...
                    if is_query_valid == True:
                        ...
                        log_values = (
                            logged_user,
                            "Consulta",
                            "Consultou um comprovante de {} na data {}, associado a conta {}.".format(
                                report_type, date, account
                            ),
                        )
                        self.query_executor.insert_query(
                            log_query,
                            log_values,
                            "Log gravado.",
                            "Erro ao gravar log:",
